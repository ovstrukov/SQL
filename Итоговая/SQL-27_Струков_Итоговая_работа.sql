...
from bookings b
	...
	full join boarding_passes bp on bp.ticket_no = tf.ticket_no
...
select f.actual_departure "Отправление", f.flight_id "ID рейса", f.flight_no "Номер рейса", (total.total_seat - reserved.res_seat) "Свободных мест", 
		...
		sum(reserved.res_seat) over (partition by f.arrival_airport, date_trunc('day', f.actual_departure) order by f.actual_departure)	"Кол-во перевезенных пассажиров"	
from flights f
